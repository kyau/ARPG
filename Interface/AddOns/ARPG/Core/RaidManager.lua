...
local manager = CreateFrame("Frame", addon.."RaidFrame", UIParent, "SecureHandlerStateTemplate")
...
function manager:CreateWorldMarkerButtons()
	for i=1, #TEX_WORLD_RAID_MARKERS do
		...
		local button = self:CreateButton(addon.."Button"..i, text, "WorldMarker"..i)
		...
		local cancelButton = self:CreateButton(addon.."Button"..i.."Cancel", "|TInterface\\Buttons\\UI-GroupLoot-Pass-Up:14:14:0:0|t", "Clear WorldMarker"..i)
...
local function DisableBlizzard()
	...
	if needReload then
		...
		StaticPopupDialogs["RRAIDMANAGER_RELOADUI_REQUEST"] = {
			text = "ARPG needs a reload to fully disable the Blizzard raid addons. Reload now?",
			button1 = "Yes",
			button2 = "No",
			OnAccept = function()
				ReloadUI()
...
button = manager:CreateButton(addon.."ButtonWMCancel", "|TInterface\\Buttons\\UI-GroupLoot-Pass-Up:14:14:0:0|t", "Clear all world markers")
button:SetScript("OnClick", ClearRaidMarker)
button:SetPoint("TOP", previousButton, "BOTTOM", 0, 0)
previousButton = button
--rolecheck button
button = manager:CreateButton(addon.."ButtonRoleCheck", "|TInterface\\LFGFrame\\LFGRole:14:14:0:0:64:16:32:48:0:16|t", "Role check")
button:SetScript("OnClick", InitiateRolePoll)
button:SetPoint("TOP", previousButton, "BOTTOM", 0, -10)
previousButton = button
--raid to party button
leftButton = manager:CreateButton(addon.."ButtonRaidToParty", "|TInterface\\GroupFrame\\UI-Group-AssistantIcon:14:14:0:0|t", "Raid to party")
leftButton:SetScript("OnClick", ConvertToParty)
leftButton:SetPoint("RIGHT", button, "LEFT", 0, 0)
--readycheck button
button = manager:CreateButton(addon.."ButtonReady", "|TInterface\\RaidFrame\\ReadyCheck-Ready:14:14:0:0|t", "Ready check")
button:SetScript("OnClick", DoReadyCheck)
button:SetPoint("TOP", previousButton, "BOTTOM", 0, 0)
previousButton = button
--party to raid button
leftButton = manager:CreateButton(addon.."ButtonPartyToRaid", "|TInterface\\GroupFrame\\UI-Group-LeaderIcon:14:14:0:0|t", "Party to raid")
leftButton:SetScript("OnClick", ConvertToRaid)
leftButton:SetPoint("RIGHT", button, "LEFT", 0, 0)
--pull button
button = manager:CreateButton(addon.."ButtonPullCounter", "|TInterface\\TargetingFrame\\UI-TargetingFrame-Skull:14:14:0:0|t", "Boss pull in 6")
button:SetPoint("TOP", previousButton, "BOTTOM", 0, 0)
button:SetAttribute("type", "macro")
button:SetAttribute("macrotext", format("/pull %d", 6))
previousButton = button
--stopwatch toggle
leftButton = manager:CreateButton(addon.."ButtonStopWatch", "|TInterface\\ChatFrame\\UI-ChatIcon-ArmoryChat-AwayMobile:14:14:0:0|t", "Toggle stopwatch")
...
bg:SetAlpha(0.05)
toggleButton.bg = bg
toggleButton.tooltipText = "Toggle "..addon

...
--[[
  ...
  local createActionBarBackground = function(self)
    ...
    local f = CreateFrame("Frame","ARPG_UnitFramesActionBarBackground",self)
    ...
    local setupBarTexture = function()
      ...
      or UnitHasVehicleUI("player") then
        t:SetTexture("Interface\\AddOns\\ARPG\\Media\\Textures\\UnitFrames\\vehiclebar")
      elseif MultiBarBottomRight:IsShown() then
        t:SetTexture("Interface\\AddOns\\ARPG\\Media\\Textures\\UnitFrames\\kyaubar")
  ...
  local createAngelFrame = function(self)
    ...
    local f = CreateFrame("Frame","ARPG_UnitFramesAngelFrame",self)
    f:SetSize(320,160)
    ...
    t:SetAllPoints(f)
    t:SetTexture("Interface\\AddOns\\ARPG\\Media\\Textures\\UnitFrames\\d3_angel2")
  end

  --create the demon
  local createDemonFrame = function(self)
    if not self.cfg.art.demon.show then return end
    local f = CreateFrame("Frame","ARPG_UnitFramesDemonFrame",self)
    f:SetSize(320,160)
    f:SetFrameStrata("LOW")
    f:SetFrameLevel(0)
    ...
    local t = f:CreateTexture(nil,"BACKGROUND",nil,2)
    t:SetAllPoints(f)
    t:SetTexture("Interface\\AddOns\\ARPG\\Media\\Textures\\UnitFrames\\d3_demon2")
  end
  ...
  local createBottomLine = function(self)
    ...
    if not cfg.show then return end
    local f = CreateFrame("Frame","ARPG_UnitFramesBottomLine",self)
    ...
    t:SetAllPoints(f)
    t:SetTexture("Interface\\AddOns\\ARPG\\Media\\Textures\\UnitFrames\\d3_bottom")
  end
]]
  ...
  local createGalaxy = function(frame,type,x,y,size,duration,texture,sublevel)
    ...
    t:SetTexture("Interface\\AddOns\\ARPG\\Media\\Textures\\UnitFrames\\"..texture)
    t:SetBlendMode("ADD")
  ...
  local createOrb = function(self,type)
    --print("|cffffd700>|r |cff00aa00"..addon..":|cffdedede orb starting "..type.."|r")
    --get the orb config
    local orbcfg = db.char[type]
    local name
    if type == "HEALTH" then
      name = "ARPG_UnitFramesHealthOrb"
    else
      name = "ARPG_UnitFramesPowerOrb"
    end
    ...
    if orb.type == "HEALTH" then
      ...
      glow:SetTexture("Interface\\AddOns\\ARPG\\Media\\Textures\\UnitFrames\\orb_debuff_glow")
      orb.glow = glow
    ...
    bg:SetAllPoints()
    bg:SetTexture("Interface\\AddOns\\ARPG\\Media\\Textures\\UnitFrames\\orb_back")
    orb.bg = bg
    ...
    -- TODO: WoW 7.0.3 Broke this function
    ...
    function model:Update()
      ...
      --print("|cffffd700>|r |cff00aa00"..addon..":|cffdedede updating orb:"..type.."|r")
    end
    model.type = orb.type
    ...
    spark:SetTexture("Interface\\AddOns\\ARPG\\Media\\Textures\\UnitFrames\\orb_spark")
    ...
    highlight:SetTexture("Interface\\AddOns\\ARPG\\Media\\Textures\\UnitFrames\\orb_gloss")
    ...
    values.bottom:SetPoint("CENTER", 0, 82)
    ...
    if self.cfg.absorb.show and orb.type == "HEALTH" then
      ...
      absorbBar.bg:SetTexture("Interface\\AddOns\\ARPG\\Media\\Textures\\UnitFrames\\orb_absorb_glow")
      absorbBar.bg:SetAlpha(0)

      absorbBar.texture = absorbBar:CreateTexture(nil,"OVERLAY",nil,-8)
      absorbBar.texture:SetPoint("BOTTOMLEFT")
      absorbBar.texture:SetPoint("BOTTOMRIGHT")
      ...
      absorbBar.PostUpdate = function(self,unit,absorb,maxHealth)
        self.texture:SetTexCoord(0,1,absorb/maxHealth,0)
      ...
      absorbBar:HookScript("OnValueChanged", function(bar,absorb)
        ...
        bar.texture:SetTexCoord(0,1,absorb/maxHealth,0)
        --bar.texture:SetTexCoord(0,1,1,0)
        bar.texture:SetHeight(absorbBar.texture.maxHeight*absorb/maxHealth)
        --bar.texture:SetHeight(absorbBar.texture.maxHeight*1)
      end)
    ...
    --print("|cffffd700>|r |cff00aa00"..addon..": |cffdededeorb created "..type.."|r")
  end
  ...
  local createStyle = function(self)
    ...
    if self.cfg.icons.resting.show then
      ...
      --local posX = pos.x + 150
      --local posY = pos.y - 44
      local posX = pos.x
      local posY = pos.y
      --self.Resting = func.createIcon(self,"DIALOG",22,self,pos.a1,pos.a2,posX,posY,-1)
      --local RestingIndicator = self:CreateTexture(nil, 'OVERLAY', nil, 7)
      local RestingIndicator = ARPG_UnitFramesHealthOrbScrollFrameOverlay:CreateTexture(nil, 'OVERLAY')
      RestingIndicator:SetSize(24, 24)
      RestingIndicator:SetPoint('TOPLEFT', self, posX, posY)
      self.RestingIndicator = RestingIndicator
    end
    if self.cfg.icons.pvp.show then
      local pos = self.cfg.icons.pvp.pos
      local posX = pos.x + 364
      local posY = pos.y + 50
      self.PvP = func.createIcon(self,"DIALOG",48,self,pos.a1,pos.a2,posX,posY,-1)
    end
    if self.cfg.icons.combat.show then
      local pos = self.cfg.icons.combat.pos
      --local posX = pos.x + 15
      --local posY = pos.y - 134
      local posX = pos.x
      local posY = pos.y
  ...
  if cfg.units.player.show then
    ...
    oUF:Spawn("player", "ARPG_UnitFramesPlayerFrame")
  end

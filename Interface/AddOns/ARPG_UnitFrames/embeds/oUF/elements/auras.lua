...
local function updateIcon(element, unit, index, offset, filter, isDebuff, visible)
	...
	if(name) then
		--print(element:GetDebugName())
		if (string.match(element:GetDebugName(), "ARPG_UnitFramesRaidGroup")) then
			local _, __class = UnitClass("player")
			--print(__class)
			if (__class == "WARRIOR") then
				if (spellID ~= 6673) then
					return HIDDEN
				end
			elseif (__class == "PALADIN") then
				-- Paladin Buffs
				return HIDDEN -- hide all
			elseif (__class == "HUNTER") then
				-- Hunter Buffs
				return HIDDEN -- hide all
			elseif (__class == "ROGUE") then
				-- Rogue Buffs
				return HIDDEN -- hide all
			else
				-- Classes without buffs hide all
				return HIDDEN -- hide all
			end
		end
